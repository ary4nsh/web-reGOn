...
func CspHeader(raw string, port ...string) {
	portNumber := "443"
	if len(port) > 0 && port[0] != "" {
		portNumber = port[0]
	}

	// build canonical URL once
	u := raw
	...
	// add port only if host part lacks one
	host := strings.Split(strings.Split(u, "://")[1], "/")[0]
	if !strings.Contains(host, ":") {
		u = strings.TrimSuffix(u, "/") + ":" + portNumber
	}

	// fire the request with the *new* URL
