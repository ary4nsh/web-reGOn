...
func main() {
	...
	var rootCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if flags.dnsFlag {
				if flags.dnsResolver == "" && flags.dnsResolver != "" {
					fmt.Println("Please provide a DNS resolver")
					return
				}
			}
			...
			functions := map[bool]func(){
				...
				flags.dnsFlag: func() {
					wg.Add(1)
					defer wg.Done()
					libs.DnsRecords(URL, flags.dnsResolver, &wg)
				},
	...
	rootCmd.Flags().StringVarP(&flags.dnsResolver, "resolver", "R", "", "DNS resolver to use (e.g., 8.8.8.8)")
