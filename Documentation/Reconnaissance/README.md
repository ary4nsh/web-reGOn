...
## WAF Detection
Use the `--waf` flag to detect a Web Application Firewall. You can also use the `--port` option to specify the port to test (if not specified, the default port is 443):
```
